...
function rpGetResearchPlan(PDO $crad, int $groupId): ?array
{
    ...
    $plan['current_stage'] = rpSynchronizeResearchPlanCurrentStage(
        $crad,
        (int) $plan['id'],
        $groupId
    );
...
/**
 * Resolve the display stage from the authoritative workflow records.
 * Second-semester enrollment takes precedence, followed by the group phase.
 * Milestone-derived values are only a safe fallback for legacy records.
 *
 * @param array<int,array<string,mixed>>|null $milestones
 */
function rpResolveResearchPlanCurrentStage(PDO $crad, int $groupId, ?array $milestones = null): string
{
    if ($groupId <= 0) {
        return 'Planning';
    }
    ...
    try {
        $secondSemester = $crad->prepare(
            "SELECT rgt.current_phase
             FROM research_group_terms rgt
             INNER JOIN academic_terms at ON at.id = rgt.academic_term_id
             WHERE rgt.research_group_id = ?
               AND rgt.status = 'Active'
               AND at.status = 'Active'
               AND at.semester = '2nd Semester'
             ORDER BY rgt.id DESC
             LIMIT 1"
        );
        $secondSemester->execute([$groupId]);
        $phase = trim((string) ($secondSemester->fetchColumn() ?: ''));
        if ($phase !== '') {
            return $phase;
        }
    } catch (Throwable $exception) {
        // Legacy installs may not have semester tables yet; continue to first-semester state.
    }

    try {
        $groupStatement = $crad->prepare('SELECT current_phase FROM research_groups WHERE id = ? LIMIT 1');
        $groupStatement->execute([$groupId]);
        $phase = trim((string) ($groupStatement->fetchColumn() ?: ''));
        if ($phase !== '') {
            return $phase;
        }
    } catch (Throwable $exception) {
        // Fall through to milestone evidence for older schemas.
    }

    if ($milestones === null) {
        try {
            $milestoneStatement = $crad->prepare(
                'SELECT milestone_order, status, progress_percentage '
                . 'FROM research_milestones WHERE research_plan_id = '
                . '(SELECT id FROM research_plans WHERE research_group_id = ? LIMIT 1) '
                . 'ORDER BY milestone_order ASC'
            );
            $milestoneStatement->execute([$groupId]);
            $milestones = $milestoneStatement->fetchAll(PDO::FETCH_ASSOC) ?: [];
        } catch (Throwable $exception) {
            $milestones = [];
        }
    }
    ...
    foreach ($milestones as $milestone) {
        ...
        $hasProgress = $hasProgress || $progress > 0 || $status !== 'Not Started';
        $hasPostPreOralProgress = $hasPostPreOralProgress
            || ($order >= 4 && ($progress > 0 || $status !== 'Not Started'));
    }

    if ($requiredComplete === 8) {
        return 'First Semester Completion';
    }
    if ($hasPostPreOralProgress) {
        return 'Post Pre-Oral Development';
    }
    return $hasProgress ? 'Research Development' : 'Planning';
}

/**
 * Persist the current workflow stage so dashboards and reports never remain at
 * the research_plans default after the group has progressed.
 *
 * @param array<int,array<string,mixed>>|null $milestones
 */
function rpSynchronizeResearchPlanCurrentStage(
    PDO $crad,
    int $planId,
    int $groupId,
    ...
    $stage = rpResolveResearchPlanCurrentStage($crad, $groupId, $milestones);
    $statement = $crad->prepare(
        'UPDATE research_plans SET current_stage = ?, updated_at = NOW() '
        . 'WHERE id = ? AND (current_stage IS NULL OR current_stage <> ?)'
    );
    $statement->execute([$stage, $planId, $stage]);
    return $stage;
}
...
{
    require_once __DIR__ . '/final-readiness-helpers.php';
    frEnsureSchema($crad);
}

function rpMigrateLegacyFinalDefenseRecommendations(PDO $crad): void
{
    // One-way compatibility migration. The dedicated recommendation table is the
    // source of truth; legacy research_plans columns are read only when no
    // dedicated record has ever been created for the group.
    $legacyColumns = [
    ...
    $hasLegacyColumns = cradSecondSemesterTableExists($crad, 'research_plans')
        && cradSecondSemesterTableExists($crad, 'final_defense_recommendations');
    foreach ($legacyColumns as $column) {
        $hasLegacyColumns = $hasLegacyColumns
            && cradSecondSemesterColumnExists($crad, 'research_plans', $column);
    }

    if ($hasLegacyColumns) {
        $crad->exec(
            "INSERT INTO final_defense_recommendations "
            . "(research_group_id, group_number, academic_term_id, adviser_user_id, adviser_name, status, remarks, recommended_at) "
            . "SELECT rp.research_group_id, COALESCE(rg.group_number, ''), "
            . "(SELECT rgt.academic_term_id FROM research_group_terms rgt "
            . " WHERE rgt.research_group_id = rp.research_group_id AND rgt.status = 'Active' "
            . " ORDER BY rgt.id DESC LIMIT 1), rp.final_defense_recommended_by, "
            . "COALESCE(rp.final_defense_recommended_by_name, ''), 'Recommended', "
            . "rp.final_defense_recommendation_remarks, rp.final_defense_recommended_at "
            . "FROM research_plans rp "
            . "LEFT JOIN research_groups rg ON rg.id = rp.research_group_id "
            . "LEFT JOIN final_defense_recommendations fdr ON fdr.research_group_id = rp.research_group_id "
            . "WHERE rp.final_defense_recommended = 1 AND fdr.id IS NULL"
        );
        $crad->exec(
            "UPDATE final_defense_recommendations fdr SET academic_term_id = ("
            . "SELECT rgt.academic_term_id FROM research_group_terms rgt "
            . "WHERE rgt.research_group_id = fdr.research_group_id AND rgt.status = 'Active' "
            . "ORDER BY rgt.id DESC LIMIT 1) "
            . "WHERE fdr.academic_term_id IS NULL AND fdr.status = 'Recommended'"
        );
    }
}
...
function rpMarkFinalDefenseRecommended(
    ...
    $groupStatement = $crad->prepare('SELECT group_number FROM research_groups WHERE id = ? LIMIT 1');
    $groupStatement->execute([$groupId]);
    $groupNumber = (string) ($groupStatement->fetchColumn() ?: '');
    if ($groupNumber === '') {
        return false;
    }
...
function rpRevokeFinalDefenseRecommendation(PDO $crad, int $groupId): bool
{
    ...
    frRevokeFinalDefenseRecommendation(
        ...
        function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? 'Adviser'),
        'Revoked through the research progress workflow.'
    );
    return true;
}
...
function rpGetFinalDefenseRecommendation(PDO $crad, int $groupId): ?array
{
    ...
    $stmt = $crad->prepare(
        "SELECT fdr.*,
                (fdr.status = 'Recommended') AS final_defense_recommended,
                fdr.adviser_user_id AS final_defense_recommended_by,
                fdr.adviser_name AS final_defense_recommended_by_name,
                fdr.recommended_at AS final_defense_recommended_at,
                fdr.remarks AS final_defense_recommendation_remarks
         FROM final_defense_recommendations fdr
         WHERE fdr.research_group_id = ? LIMIT 1"
    );
    $stmt->execute([$groupId]);
...
/**
 * Enforce the official first-semester milestone sequence on the server.
 * Chapters 1-3 may be developed in parallel before Pre-Oral. Chapter 4 starts
 * only after a completed, non-failed Pre-Oral; milestones 5-8 then proceed in
 * order and require the preceding post-Pre-Oral milestone to be approved.
 *
 * @return array{valid:bool,message:string}
 */
function rpValidateMilestoneSubmissionSequence(
    PDO $crad,
    int $groupId,
    int $planId,
    array $milestone,
    ...
    if ($newProgress < 0 || $newProgress > 100) {
        return ['valid' => false, 'message' => 'Progress must be between 0 and 100 percent.'];
    }

    $currentProgress = (float) ($milestone['progress_percentage'] ?? 0);
    if ($newProgress < $currentProgress) {
        return ['valid' => false, 'message' => 'Progress cannot be lower than the milestone\'s current approved progress.'];
    }
    if ($requestedStatus === 'Submitted for Review' && $newProgress <= 0) {
        return ['valid' => false, 'message' => 'Enter actual progress before submitting this milestone for Adviser review.'];
    }
    ...
    $panelResult = strtoupper(trim((string) ($panelRecord['final_result'] ?? '')));
    if ($panelResult === '' || $panelResult === 'FAILED') {
        $message = $panelResult === 'FAILED'
            ? 'The Pre-Oral result is FAILED. Complete the required re-defense process before continuing to Chapter 4.'
            : 'Complete all assigned Panel evaluations for the Pre-Oral Defense before continuing to Chapter 4.';
        return ['valid' => false, 'message' => $message];
    }

    if ($order >= 5) {
        $previousStatement = $crad->prepare(
            "SELECT milestone_name, status, progress_percentage
             FROM research_milestones
             WHERE research_plan_id = ? AND milestone_order = ?
             ORDER BY id ASC LIMIT 1"
        );
        $previousStatement->execute([$planId, $order - 1]);
        $previous = $previousStatement->fetch(PDO::FETCH_ASSOC) ?: null;
        if (!$previous
            || !in_array((string) ($previous['status'] ?? ''), ['Approved', 'Completed'], true)
            || (float) ($previous['progress_percentage'] ?? 0) < 100) {
            $previousName = (string) ($previous['milestone_name'] ?? ('Milestone ' . ($order - 1)));
            return [
                'valid' => false,
                'message' => 'Complete and obtain Adviser approval for ' . $previousName . ' before starting this milestone.',
            ];
        }
    }
...
function rpSyncFirstSemesterCompletionPhase(PDO $crad, int $groupId, int $planId): bool
{
    ...
    $stmt = $crad->prepare(
        "SELECT COUNT(*) AS required_count,
         ...
         FROM research_milestones
         WHERE research_plan_id = ? AND milestone_order BETWEEN 1 AND 8
    ...
    if ((int) ($counts['required_count'] ?? 0) !== 8 || (int) ($counts['completed_count'] ?? 0) !== 8) {
        return false;
    }
    $update = $crad->prepare(
        "UPDATE research_groups rg
         SET rg.current_phase = 'First Semester Completion', rg.current_phase_started_at = NOW()
         WHERE rg.id = ?
           AND NOT EXISTS (SELECT 1 FROM research_group_terms rgt WHERE rgt.research_group_id = rg.id)"
    );
    $update->execute([$groupId]);
...
{
    if ($groupId <= 0 || $chapter < 1 || $chapter > 5) {
        return null;
    }
...
function rpGetOrCreateResearchPlan(PDO $crad, int $groupId): ?array
{
    ...
    if ($existing) {
        $existing['current_stage'] = rpSynchronizeResearchPlanCurrentStage(
            $crad,
            (int) $existing['id'],
            $groupId
        );
    ...
    try {
        ...
        rpSynchronizeResearchPlanCurrentStage($crad, $planId, $groupId);
        ...
        if (strpos($e->getMessage(), 'uniq_rp_group') !== false) {
            ...
            $stmt->execute([$groupId]);
            $concurrentPlan = $stmt->fetch(PDO::FETCH_ASSOC) ?: null;
            if ($concurrentPlan) {
                $concurrentPlan['current_stage'] = rpSynchronizeResearchPlanCurrentStage(
                    $crad,
                    (int) $concurrentPlan['id'],
                    $groupId
                );
            }
...
function rpRecalculateOverallProgress(PDO $crad, int $planId): void
{
    ...
    if ($groupId > 0) {
        rpSyncFirstSemesterCompletionPhase($crad, $groupId, $planId);
    }
    $currentStage = $groupId > 0
        ? rpResolveResearchPlanCurrentStage($crad, $groupId, $milestones)
        : 'Planning';
    ...
    $updateStmt = $crad->prepare("
        ...
        WHERE id = ?
    ");
    $updateStmt->execute([$avgProgress, $currentStage, $planId]);
}

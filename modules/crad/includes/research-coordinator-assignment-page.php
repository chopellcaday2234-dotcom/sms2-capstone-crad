...
function rcAssignmentSyncApprovedTitleGroups(PDO $pdo): void
{
    try {
        ...
        foreach ([
            ...
            'current_phase' => "ALTER TABLE research_groups ADD current_phase VARCHAR(80) NOT NULL DEFAULT 'Research Development' AFTER created_by",
            'current_phase_started_at' => "ALTER TABLE research_groups ADD current_phase_started_at DATETIME DEFAULT NULL AFTER current_phase",
        ] as $column => $sql) {
            ...
            $ins = $pdo->prepare("
                INSERT INTO research_groups
                    (proposal_id, title_approval_id, proposal_number, group_number, group_name,
                     ...
                     status, date_assigned, created_by, current_phase, current_phase_started_at)
                VALUES
                    (NULL, :title_approval_id, :proposal_number, :group_number, :group_name,
                     :research_title, :college_dept, :adviser, :academic_year,
                     :leader_name, :leader_id, '', '',
                     'Approved', :date_assigned, :created_by, 'Research Development', NOW())
            ");
            $ins->execute([
...
function rcAssignmentResetStaleAssignments(PDO $pdo): void
{
    ...
    foreach ($defenseTables as $defenseTable) {
        try {
            ...
            // A completed Pre-Oral schedule ends only the first-semester defense,
            // not the adviser relationship. Keep the same active adviser through
            // the second-semester draft, readiness, Final Defense, and approval flow.
            $activeSecondSemesterAssignment = "
                NOT EXISTS (
                    SELECT 1
                    FROM research_group_terms active_rgt
                    INNER JOIN academic_terms active_at
                      ON active_at.id = active_rgt.academic_term_id
                    INNER JOIN research_groups active_rg
                      ON active_rg.id = active_rgt.research_group_id
                    WHERE active_rgt.status = 'Active'
                      AND active_at.status = 'Active'
                      AND active_at.semester = '2nd Semester'
                      AND active_rgt.current_phase NOT IN ('Publication and Repository', 'Completed')
                      AND (
                            (a.research_group_id IS NOT NULL AND a.research_group_id = active_rgt.research_group_id)
                         OR (a.group_number IS NOT NULL AND a.group_number <> '' AND a.group_number = active_rg.group_number)
                      )
                )
            ";
            ...
            $pdo->exec("
                UPDATE research_adviser_assignments a
                 ...
                 WHERE a.assignment_status = 'Assigned'
                   ...
                   AND {$activeSecondSemesterAssignment}
            ");
        } catch (Throwable $e) {

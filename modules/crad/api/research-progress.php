...
// API responses must remain valid JSON. Runtime diagnostics are written to
// the PHP log instead of being injected into fetch() responses.
...
function handleSubmitProgress(PDO $crad, int $groupId, array $researchGroup, int $userId, string $userName): void
{
    ...
    $sequenceCheck = rpValidateMilestoneSubmissionSequence(
        $crad,
        $groupId,
        (int) $plan['id'],
        $milestone,
        (float) $input['new_progress'],
        $milestoneStatus
    );
    if (!$sequenceCheck['valid']) {
        http_response_code(409);
        echo json_encode(['success' => false, 'message' => $sequenceCheck['message']]);
        return;
    }

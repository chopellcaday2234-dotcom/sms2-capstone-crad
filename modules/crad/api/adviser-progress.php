...
function handleRequestRevision(PDO $crad, int $adviserUserId, string $adviserEmail, string $adviserName): void
{
    ...
    try {
        ...
        if (!empty($update['milestone_id'])) {
            ...
            rpSyncFirstSemesterCompletionPhase(
                $crad,
...
function handleApproveProgress(PDO $crad, int $adviserUserId, string $adviserEmail, string $adviserName): void
{
    ...
    if ((float) ($update['new_progress'] ?? 0) < 100) {
        http_response_code(409);
        echo json_encode([
            'success' => false,
            'message' => 'Only a 100% milestone submission can be finalized as Approved. Request a revision or wait for the student to complete it.',
        ]);
        return;
    }
    ...
    try {
        ...
        if (!empty($update['milestone_id'])) {
            ...
            if ($isChapter123) {
                ...
                $panelResult = (string) ($panelRecord['final_result'] ?? '');
                $panelApproved = $panelResult === 'APPROVED';
                $revisionAfterDefense = $panelResult === 'APPROVED WITH REVISION'
                    && !empty($panelRecord['approved_at'])
                    && (string) ($update['submitted_at'] ?? '') >= (string) $panelRecord['approved_at'];

                if ($panelApproved || $revisionAfterDefense) {
                    ...
                    $updateMilestone = $crad->prepare("
                        ...
                        SET progress_percentage = 100,
                            ...
                            adviser_remarks     = ?,
                            panel_remarks       = COALESCE(NULLIF(panel_remarks, ''), ?),
                            updated_at          = NOW()
                        WHERE id = ?
                    ...
                    $updateMilestone->execute([
                        ...
                        (string) ($panelRecord['panel_remarks'] ?? ''),
                        $update['milestone_id'],
                        $update['research_plan_id'],
                    ]);
                } else {

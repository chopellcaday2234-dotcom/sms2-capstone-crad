...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfVerify()) { $publicationError = 'Security check failed.'; }
        ...
        if (($_POST['publication_action'] ?? '') === 'create') {
            ...
            $groupStmt = $crad->prepare("SELECT rg.research_title, rg.group_name, fma.id AS final_approval_id FROM research_groups rg INNER JOIN final_manuscript_approvals fma ON fma.research_group_id = rg.id AND fma.status = 'Approved' WHERE rg.id = ? LIMIT 1");
            ...
            if (!$group || !fpIsFinalManuscriptApproved($crad, $groupId)) {
                ...
                if ($check->fetch()) {
                    ...
                    $stmt = $crad->prepare("INSERT INTO publications (research_group_id, final_approval_id, title, authors, status, created_by_user, created_by_name) VALUES (?, ?, ?, ?, 'Draft', ?, ?)");
                    $stmt->execute([$groupId, (int) $group['final_approval_id'], $group['research_title'], $group['group_name'], (int) ($_SESSION['user_id'] ?? 0), function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? 'CRAD User')]);

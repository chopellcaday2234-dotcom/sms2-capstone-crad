...
function cradPruneOrphanDefenseSchedules(PDO $pdo): void
{
    ...
    // Retain defense schedules as academic/audit records even when the original
    // proposal row has been superseded by an official research-group record.
    $pdo->exec("
        UPDATE research_defense_schedules rds
        INNER JOIN research_groups rg
            ON rg.group_number = rds.group_number
            OR (
                rds.proposal_number IS NOT NULL
                AND rds.proposal_number <> ''
                AND rg.proposal_number = rds.proposal_number
            )
        SET rds.research_group_id = rg.id
        WHERE rds.research_group_id IS NULL
    ");
}
...
function cradRecordDefenseSchedule(PDO $pdo, string $groupNumber, string $proposalRef): ?array
{
    ...
    $insert = $pdo->prepare("
        ...
        ON DUPLICATE KEY UPDATE
            ...
            panel_members = COALESCE(NULLIF(VALUES(panel_members), ''), research_defense_schedules.panel_members),
            panel_chair = COALESCE(NULLIF(VALUES(panel_chair), ''), research_defense_schedules.panel_chair),
            status = research_defense_schedules.status,

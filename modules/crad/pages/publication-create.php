...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfVerify()) $error = 'Security check failed.';
        ...
        $stmt = $crad->prepare("SELECT rg.research_title, rg.group_name, fma.id AS final_approval_id FROM research_groups rg INNER JOIN final_manuscript_approvals fma ON fma.research_group_id = rg.id AND fma.status = 'Approved' WHERE rg.id = ? AND NOT EXISTS (SELECT 1 FROM publications p WHERE p.research_group_id = rg.id) LIMIT 1");
        $stmt->execute([$groupId]); $group = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$group || !fpIsFinalManuscriptApproved($crad, $groupId)) $error = 'Only traceable approved groups without an existing publication record may be selected.';
        else {
            $insert = $crad->prepare("INSERT INTO publications (research_group_id, final_approval_id, title, authors, status, created_by_user, created_by_name) VALUES (?, ?, ?, ?, 'Draft', ?, ?)");
            $insert->execute([$groupId, (int) $group['final_approval_id'], $group['research_title'], $group['group_name'], (int) ($_SESSION['user_id'] ?? 0), function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? 'CRAD User')]);

...
$sqlFile = __DIR__ . '/crad_schema.sql';
...
if ($sql === false) {
    out('Unable to read schema file: ' . $sqlFile, false);
    if (!$isCli) { echo '</body></html>'; }
    exit(1);
}

/**
 * Parse a phpMyAdmin/MariaDB dump without breaking triggers or strings that
 * contain semicolons. DELIMITER is a mysql client directive, so it is consumed
 * here and is never sent to PDO.
 *
 * @return string[]
 */
function cradSqlStatements(string $sql): array
{
    ...
    foreach ($lines as $line) {
        ...
        if (!str_ends_with($candidate, $delimiter)) {
            continue;
        }
    ...
    if (trim($buffer) !== '') {
        throw new RuntimeException('The SQL schema ends with an incomplete statement.');
    }
    return $statements;
}

try {
    $statements = cradSqlStatements($sql);
} catch (Throwable $e) {
    out('Unable to parse schema file: ' . $e->getMessage(), false);
    if (!$isCli) { echo '</body></html>'; }
    exit(1);
}
...
// Apply versioned, idempotent repairs and second-semester workflow tables.
try {
    $migrationResult = cradEnsureSecondSemesterSchema($pdo);
    out('Second-semester schema ' . $migrationResult['version'] . ' is ready');
    foreach ($migrationResult['operations'] as $operation) {
        out($operation);
    }
} catch (Throwable $e) {
    out('Second-semester schema migration failed: ' . $e->getMessage(), false);
    $errors++;
}
...
$expected = [
    ...
    'academic_terms',
    'research_group_terms',
    'research_workflow_events',
    'final_readiness_checks',
    'final_draft_submissions',
    'final_draft_reviews',
    'defense_attempts',
    'defense_official_results',
    'defense_revision_items',
    'defense_revision_submissions',
];

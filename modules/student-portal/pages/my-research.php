            ...
            <div class="glass-panel-body">
                <div class="glass-panel-head">
                    <div>
                        ...
                        <p class="glass-panel-sub">Formal recommendation after adviser and CRAD readiness checks</p>
                    </div>
                ...
                <?php if (($finalDefenseRecommendation['status'] ?? '') === 'Recommended'): ?>
                    <div class="small text-muted">
                        Recommended by <strong><?= htmlspecialchars((string) ($finalDefenseRecommendation['adviser_name'] ?? '')) ?></strong>
                        <?php if (!empty($finalDefenseRecommendation['recommended_at'])): ?>
                            on <?= date('M d, Y g:i A', strtotime((string) $finalDefenseRecommendation['recommended_at'])) ?>
                        <?php endif; ?>
                    </div>
                    <?php if (trim((string) ($finalDefenseRecommendation['remarks'] ?? '')) !== ''): ?>
                        <div class="mt-2" style="white-space:pre-line;color:var(--sms-text);">
                            <?= htmlspecialchars((string) $finalDefenseRecommendation['remarks']) ?>
                        </div>
                    <?php endif; ?>
                <?php else: ?>
                    <p class="text-muted">Your adviser has not submitted the formal recommendation yet.</p>
                    <?php if (!empty($finalReadinessSnapshot['missing'])): ?>
                        <div class="small"><strong>Pending requirements:</strong><ul class="mb-2 mt-1">
                            <?php foreach ($finalReadinessSnapshot['missing'] as $missingRequirement): ?><li><?= htmlspecialchars((string) $missingRequirement) ?></li><?php endforeach; ?>
                        </ul></div>
                    <?php endif; ?>
                    <a class="btn btn-outline-primary btn-sm" href="<?= BASE_URL ?>/modules/student-portal/pages/final-manuscript.php"><i class="fas fa-file-alt me-1"></i>Open Final Documentation</a>
                <?php endif; ?>
            </div>
        </div>

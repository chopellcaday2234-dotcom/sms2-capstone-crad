...
$studentName   = trim(function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? ''));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function readJsonResponse(response) {
        ...
        try {
            ...
            // Some local PHP/XAMPP configurations print an upload-temp warning
            // before the API JSON. Recover the JSON payload so the student sees
            // the real validation message instead of a misleading network error.
            const firstBrace = text.indexOf('{');
            const lastBrace  = text.lastIndexOf('}');
            if (firstBrace >= 0 && lastBrace > firstBrace) {
                try {
                    const recovered = JSON.parse(text.slice(firstBrace, lastBrace + 1));
                    console.warn('Recovered JSON after unexpected server output.');
                    return recovered;
                } catch (_) { /* report the unreadable response below */ }
            }
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const invalidSuccess = error.message === 'invalid_success_response';
            const invalidError = error.message === 'invalid_error_response';
            const message = invalidSuccess
                ...
                : (invalidError
                    ? '<i class="fas fa-exclamation-triangle me-2"></i>The server response could not be read. Please refresh the page and try again.'
                    : '<i class="fas fa-wifi me-2"></i>Network error. Please check your connection and try again.');
            showFormAlert(invalidSuccess ? 'success' : 'danger', message);
            if (invalidSuccess) {
                setSubmittedState();
                setTimeout(function () {

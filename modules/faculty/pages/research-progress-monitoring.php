...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fdr_action'])) {
    if (!csrfVerify()) {
        ...
        try {
            ...
            $adviserName = trim(function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? ''));
            if ($action === 'recommend') {
                ...
                $recommendationMessage = $saved
                    ? 'Final Defense recommendation saved. The group may now submit the official Chapters 1–5 manuscript.'
                    : 'The Final Defense recommendation could not be saved.';
                $recommendationMessageType = $saved ? 'success' : 'danger';
            } elseif ($action === 'revoke') {
                $reason = trim((string) ($_POST['revocation_reason'] ?? ''));
                $saved = fpClearFinalDefenseRecommendation($crad, $groupId, $adviserUserId, $adviserName, $reason);
                $recommendationMessage = $saved
                    ? 'Final Defense recommendation revoked with an audit record.'
                    : 'The Final Defense recommendation could not be revoked.';
                $recommendationMessageType = $saved ? 'success' : 'danger';
            }
        } catch (Throwable $exception) {
            $recommendationMessage = $exception->getMessage();
            $recommendationMessageType = 'danger';
        }
    }
}
...
$finalReadinessSnapshot = frGetReadinessSnapshot($crad, $groupId);
    ...
    <div class="glass-board">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4 d-flex flex-column gap-4">
                ...
                <!-- Final Defense Readiness and Recommendation -->
                <div class="glass-panel">
                    <div class="glass-panel-body">
                        <div class="glass-panel-head">
                            <div>
                                <h5 class="glass-panel-title">Final Defense Recommendation</h5>
                                <p class="glass-panel-sub">Available only after adviser endorsement and CRAD readiness verification.</p>
                            </div>
                        </div>
                        ...
                        <?php if (($finalDefenseRecommendation['status'] ?? '') === 'Recommended'): ?>
                            <div class="alert alert-success py-2 mb-3">
                                ...
                                <div class="small mt-1">
                                    <?= htmlspecialchars((string) ($finalDefenseRecommendation['adviser_name'] ?? '')) ?>
                                    <?php if (!empty($finalDefenseRecommendation['recommended_at'])): ?>
                                        <br><?= date('M d, Y g:i A', strtotime((string) $finalDefenseRecommendation['recommended_at'])) ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <?php if (trim((string) ($finalDefenseRecommendation['remarks'] ?? '')) !== ''): ?>
                                <p class="small text-muted mb-3"><?= nl2br(htmlspecialchars((string) $finalDefenseRecommendation['remarks'])) ?></p>
                            <?php endif; ?>
                            <form method="post">
                                <?= csrfField() ?>
                                <input type="hidden" name="fdr_action" value="revoke">
                                <label class="form-label small fw-bold" for="revocation_reason">Revocation reason</label>
                                <textarea class="form-control form-control-sm mb-2" id="revocation_reason" name="revocation_reason" rows="2" required placeholder="Explain why the recommendation must be revoked."></textarea>
                                <button type="submit" class="btn btn-outline-danger btn-sm">
                                    <i class="fas fa-undo me-1"></i>Revoke Recommendation
                                </button>
                            </form>
                        <?php else: ?>
                            <?php if ($finalReadinessSnapshot['ready']): ?>
                                <div class="alert alert-success py-2"><i class="fas fa-clipboard-check me-1"></i>All adviser and CRAD requirements are complete.</div>
                                <form method="post">
                                    <?= csrfField() ?>
                                    <input type="hidden" name="fdr_action" value="recommend">
                                    <label class="form-label small fw-bold" for="fdr_remarks">Recommendation remarks (optional)</label>
                                    <textarea class="form-control form-control-sm mb-3" id="fdr_remarks" name="fdr_remarks" rows="3" maxlength="5000" placeholder="Add final readiness remarks."></textarea>
                                    <button type="submit" class="btn btn-primary btn-sm">
                                        <i class="fas fa-flag-checkered me-1"></i>Recommend for Final Defense
                                    </button>
                                </form>
                            <?php else: ?>
                                <div class="alert alert-warning py-2 mb-2"><strong>Recommendation blocked.</strong></div>
                                <?php if (!empty($finalReadinessSnapshot['missing'])): ?>
                                    <ul class="small text-muted ps-3 mb-3">
                                        <?php foreach ($finalReadinessSnapshot['missing'] as $missingRequirement): ?>
                                            <li><?= htmlspecialchars((string) $missingRequirement) ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                <?php endif; ?>
                                <a class="btn btn-outline-primary btn-sm" href="<?= BASE_URL ?>/modules/faculty/pages/final-draft-review.php?group=<?= urlencode($groupNumber) ?>">
                                    <i class="fas fa-file-alt me-1"></i>Open Final Draft Review
                                </a>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>

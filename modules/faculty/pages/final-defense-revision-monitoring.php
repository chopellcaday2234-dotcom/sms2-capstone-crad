...
$adviserName = function_exists('getCurrentUserName') ? getCurrentUserName() : (string) ($_SESSION['user_name'] ?? 'Adviser');
$message = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfVerify()) {
        ...
        try {
            if (fpSetFinalDefenseRevisionStatus($crad, $groupId, $adviserUserId, $adviserEmail, $status, $adviserName, $remarks)) {
                $message = 'Final Defense revision compliance status updated.';
            } else {
                $error = 'Unable to update this revision cycle. A dedicated revision file must be submitted first.';
            }
                        ...
                        <div class="glass-panel h-100"><div class="glass-panel-body">
                            ...
                            <div class="alert <?= !empty($group['revision_submitted']) ? 'alert-info' : 'alert-warning' ?> py-2 small">
                                ...
                                <?php if (!empty($group['revision_submitted'])): ?>
                                    Revision file: <a target="_blank" href="<?= BASE_URL ?>/modules/crad/api/defense-revision-document.php?id=<?= (int) $group['revision_update_id'] ?>"><?= htmlspecialchars((string) ($group['revision_update_title'] ?: 'Revision evidence')) ?></a>
                                ...
                                <?php else: ?>
                                    Waiting for the researcher to submit a dedicated Final Defense revision file.
                                <?php endif; ?>
                            </div>
                            ...
                            <form method="post" class="row g-2 align-items-end mt-3">
                                ...
                                <div class="col-md-4"><label class="form-label small">Decision</label><select class="form-select form-select-sm" name="revision_status" aria-label="Revision compliance status">
                                ...
                                <div class="col-md-5"><label class="form-label small">Review Remarks</label><input class="form-control form-control-sm" name="review_remarks" placeholder="Required for revision/compliant decisions"></div>
                                <div class="col-md-3"><button class="btn btn-primary btn-sm w-100" type="submit"><i class="fas fa-save me-1"></i>Save Status</button></div>
                            </form>

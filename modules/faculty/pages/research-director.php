...
/**
 * Resolve the defense phase from the active workflow instead of hard-coding
 * every Research Director schedule as Pre-Oral Defense.
 */
function rdScheduleDefenseType(PDO $pdo, int $groupId): string
{
    if ($groupId < 1) {
        return 'Pre-Oral Defense';
    }
    ...
    $phaseStmt->execute([$groupId]);
    $currentPhase = trim((string) $phaseStmt->fetchColumn());
    if (!in_array($currentPhase, ['Final Defense Scheduling', 'Final Defense'], true)) {
        return 'Pre-Oral Defense';
    }

    $failedAttempt = $pdo->prepare(
        "SELECT 1
         ...
         WHERE da.research_group_id = ?
           ...
           AND dor.official_result = 'FAILED'
           AND dor.finalized_at IS NOT NULL
         LIMIT 1"
    );
    $failedAttempt->execute([$groupId]);
    return $failedAttempt->fetchColumn() ? 'Re-Defense' : 'Final Defense';
}

function rdScheduleAcademicTermId(PDO $pdo, int $groupId): ?int
{
    $stmt = $pdo->prepare(
        "SELECT rgt.academic_term_id
         FROM research_group_terms rgt
         INNER JOIN academic_terms at ON at.id = rgt.academic_term_id
         WHERE rgt.research_group_id = ?
           AND rgt.status = 'Active'
...
function rdScheduleIsFinalPhase(string $defenseType): bool
{
    return in_array($defenseType, ['Final Defense', 'Re-Defense'], true);
}

function rdScheduleTypeWhere(string $alias = 'research_defense_schedules'): string
{
    return "LOWER(TRIM(COALESCE({$alias}.defense_type, '')))";
}
...
if ($crad) {
    ...
    if (in_array($view, ['manual-scheduling-optimizer', 'alternative-time-slots'], true)
        ...
        if (!csrfVerify()) {
            ...
            $defenseType = $groupId > 0 ? rdScheduleDefenseType($crad, $groupId) : 'Pre-Oral Defense';
            $academicTermId = rdScheduleIsFinalPhase($defenseType)
                ? rdScheduleAcademicTermId($crad, $groupId)
                : null;
            ...
            if (rdScheduleIsFinalPhase($defenseType) && $academicTermId === null) {
                $errors[] = 'The active 2nd Semester enrollment could not be verified for Final Defense scheduling.';
            }
                ...
                $official = $crad->prepare(
                    "SELECT id FROM research_defense_schedules
                     WHERE research_group_id = ?
                       AND " . (rdScheduleIsFinalPhase($defenseType)
                           ? rdScheduleTypeWhere() . " IN ('final defense', 're-defense')"
                           : rdScheduleTypeWhere() . " NOT IN ('final defense', 're-defense')") . "
            ...
            if ($errors) {
                ...
                sort($slotSignatures);
                $scheduleLockName = 'rd_schedule_' . $groupId . '_' . sha1($defenseType . '|' . implode('|', $slotSignatures));
                $lockAcquired = false;
                try {
                    ...
                    $insert = $crad->prepare(
                        "INSERT INTO research_defense_schedules
                            (research_group_id, proposal_id, proposal_number, group_number, research_group,
                             ...
                             defense_datetime, defense_end_datetime, defense_type, status, recorded_by,
                             academic_term_id, recorded_at, updated_at)
                         VALUES
                            (:research_group_id, :proposal_id, :proposal_number, :group_number, :research_group,
                             :research_title, :adviser_name, :panel_members, :panel_chair, :venue, :venue_id,
                             :defense_datetime, :defense_end_datetime, :defense_type, 'Proposed', :recorded_by,
                             :academic_term_id, NOW(), NOW())"
                    );
    ...
    if ($view === 'finalize-defense-schedule'
        ...
        try {
            ...
            $groupId = (int) ($slot['research_group_id'] ?? 0);
            $defenseType = trim((string) ($slot['defense_type'] ?? 'Pre-Oral Defense'));
            if (strcasecmp($defenseType, 'Pre-Oral') === 0) {
                $defenseType = 'Pre-Oral Defense';
            }
            $isFinalDefense = rdScheduleIsFinalPhase($defenseType);
            $typeFilter = $isFinalDefense
                ? rdScheduleTypeWhere() . " IN ('final defense', 're-defense')"
                : rdScheduleTypeWhere() . " NOT IN ('final defense', 're-defense')";
            ...
            if ($isFinalDefense) {
                $clonePanels = $crad->prepare(
                    "INSERT INTO research_panel_assignments
                        (research_group_id, defense_schedule_id, proposal_id, title_approval_id,
                         proposal_number, group_number, research_title, panel_user_id, panel_name,
                         panel_email, expertise, availability_status, assignment_status,
                         defense_phase, assigned_by, assigned_at, created_at, updated_at)
                     SELECT research_group_id, ?, proposal_id, title_approval_id,
                            proposal_number, group_number, research_title, panel_user_id, panel_name,
                            panel_email, expertise, availability_status, 'Assigned',
                            'Final Defense', ?, NOW(), NOW(), NOW()
                     FROM research_panel_assignments
                     WHERE research_group_id = ?
                       AND defense_phase = 'Pre-Oral Defense'
                       AND assignment_status = 'Assigned'
                     ON DUPLICATE KEY UPDATE
                        defense_schedule_id = VALUES(defense_schedule_id),
                        ...
                        panel_email = VALUES(panel_email),
                ...
                $clonePanels->execute([$scheduleId, (int) getCurrentUserId(), $groupId]);
            }
            $panelAssignmentFilter = $isFinalDefense
                ? "rpa.defense_phase = 'Final Defense' AND rpa.assignment_status = 'Assigned'"
                : rdPanelActiveAssignmentSql('rpa');
            ...
            $crad->prepare("UPDATE research_defense_schedules SET status = 'Rejected', updated_at = NOW() WHERE research_group_id = ? AND id <> ? AND {$typeFilter} AND LOWER(status) IN ('proposed', 'selected')")
            ...
            $crad->prepare(
                "UPDATE research_panel_assignments
                 ...
                 WHERE research_group_id = ?
                   AND " . ($isFinalDefense
                       ? "research_panel_assignments.defense_phase = 'Final Defense' AND research_panel_assignments.assignment_status = 'Assigned'"
                       : rdPanelActiveAssignmentSql('research_panel_assignments'))
            )->execute([$scheduleId, $groupId]);
            if ($isFinalDefense) {
                $groupTerm = finalDefenseActiveGroupTerm($crad, $groupId);
                $slot['academic_term_id'] = (int) $groupTerm['academic_term_id'];
                $slot['defense_type'] = $defenseType;
                finalDefenseEnsureAttempt($crad, $slot, $groupTerm, (int) getCurrentUserId());
            }
            $notify = $crad->prepare(
            ...
            foreach ($assignedPanels as $panel) {
                ...
                $notify->execute([
                    ':event_key' => ($isFinalDefense ? 'final-defense-finalized' : 'preoral-defense-finalized') . ':s' . $scheduleId . ':u' . $panelUserId,
                    ':recipient_user_id' => $panelUserId,
                    ':recipient_email' => (string) ($panel['panel_email'] ?? ''),
                    ':panel_assignment_id' => (int) ($panel['id'] ?? 0) ?: null,
                    ':research_group_id' => $groupId,
                    ':title' => $defenseType . ' Scheduled',
                    ':body' => $notificationBody,
                    ':url' => $isFinalDefense
                        ? BASE_URL . '/modules/faculty/pages/panel-final-defense-evaluation.php?id=' . $scheduleId
                        : BASE_URL . '/modules/faculty/pages/defense-details.php?id=' . $scheduleId,
                ]);
            }
            $crad->commit();
            echo json_encode(['ok' => true, 'message' => $defenseType . ' schedule confirmed.']);
        } catch (Throwable $e) {
            if ($crad->inTransaction()) {
                $crad->rollBack();
            }
...
$selectedDefenseType = ($crad && $selectedGroupId > 0)
    ? rdScheduleDefenseType($crad, $selectedGroupId)
    : 'Defense';
...
if ($crad && $selectedGroupId > 0) {
    try {
        $officialStmt = $crad->prepare(
            "SELECT id FROM research_defense_schedules
             WHERE research_group_id = ?
               AND " . (rdScheduleIsFinalPhase($selectedDefenseType)
                   ? rdScheduleTypeWhere() . " IN ('final defense', 're-defense')"
                   : rdScheduleTypeWhere() . " NOT IN ('final defense', 're-defense')") . "
               AND LOWER(status) IN ('scheduled', 'finalized', 'final')
             LIMIT 1"
...
<?php if ($isSchedulerView): ?>
    <section class="director-scheduler">
        <div class="director-scheduler__head">
            ...
            <p><?= $selectedReadyGroup ? 'Create three proposed ' . htmlspecialchars($selectedDefenseType) . ' slots from current database records.' : 'Select a defense-ready research before creating a defense schedule.' ?></p>
        </div>
        <?php if (!$hasExplicitGroupSelection): ?>
            <div class="director-scheduler-grid">
                <div class="director-scheduler-box">
                    ...
                    <p>Please select a defense-ready research before creating a defense schedule.</p>
        ...
        <?php elseif (!$selectedReadyGroup): ?>
            <div class="director-scheduler-grid">
                <div class="director-scheduler-box">
                    ...
                    <p><?= !$selectedGroupIsOfficial ? 'This group can no longer be used in defense scheduling.' : 'One or more required records are no longer complete.' ?></p>
                    <a class="director-action-btn" href="<?= htmlspecialchars(rdScheduleUrl('defense-scheduling-queue')) ?>">
        ...
        <?php else: ?>
            <div class="director-scheduler-grid">
                <div class="director-scheduler-box">
                    ...
                    <small>Defense Phase: <?= htmlspecialchars($selectedDefenseType) ?></small>
...
<?php if ($view === 'finalize-defense-schedule'): ?>
    <div class="director-confirm" data-finalize-modal aria-hidden="true">
        <div class="director-confirm__box" role="dialog" aria-modal="true" aria-labelledby="finalize-title">
            <h2 id="finalize-title" style="font-size:1rem; font-weight:800; margin:0 0 .45rem;">Finalize Defense Schedule</h2>

...
function panelSubmitEvaluation(int $scheduleId, array $data): array
{
    ...
    try {
        ...
        $assignedStatement = $crad->prepare(
        ...
        $assignedStatement->execute([(int) ($defense['research_group_id'] ?? 0)]);
        $requiredCount = (int) $assignedStatement->fetchColumn();

        $resultsStatement = $crad->prepare(
            "SELECT pde.result
             FROM preoral_defense_evaluations pde
             INNER JOIN research_panel_assignments rpa
               ON rpa.research_group_id = pde.research_group_id
              AND rpa.panel_user_id = pde.panel_user_id
              AND rpa.defense_phase = 'Pre-Oral Defense'
              AND rpa.assignment_status = 'Assigned'
             WHERE pde.defense_schedule_id = ? AND pde.status = 'Submitted'
             GROUP BY pde.panel_user_id, pde.result
             ORDER BY MIN(pde.id) ASC"
        );
        $resultsStatement->execute([$scheduleId]);
        $submittedResults = $resultsStatement->fetchAll(PDO::FETCH_COLUMN) ?: [];

        if ($requiredCount > 0 && count($submittedResults) >= $requiredCount) {
            $officialResult = panelFinalResultFromResults($submittedResults);
            $scheduleStatus = $officialResult === 'FAILED' ? 'Failed' : 'Completed';
            $groupPhase = $officialResult === 'FAILED' ? 'Pre-Oral Failed' : 'Post Pre-Oral Development';
            $crad->prepare(
                'UPDATE research_defense_schedules SET status = ?, updated_at = NOW() WHERE id = ?'
            )->execute([$scheduleStatus, $scheduleId]);
            $crad->prepare(
                'UPDATE research_groups SET current_phase = ?, current_phase_started_at = NOW() WHERE id = ?'
            )->execute([$groupPhase, (int) ($defense['research_group_id'] ?? 0)]);
        }
        $crad->commit();
